...
class Service(object):
    ...
    async def execute(self, plugin, command, blocking=True, outfile=None, errfile=None, future_outfile=None):
        ...
        self.logger.info(
            'Service scan {bblue}' + plugin.name + ' {green}(' + tag + '){rst} is running the following command against {byellow}' + self.target.address + '{rst}: ' + cmd, verbosity=2)
    ...
    def add_vulnerability(self, vuln_id, proofs, plugin, name=None):
        ...
        if not vuln_exists:
            ...
            self.logger.info("Vulnerability found: %s by plugin %s" % (vuln.name, 
