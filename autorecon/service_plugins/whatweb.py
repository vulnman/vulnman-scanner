...
class Whatweb(servicescan.ServiceScan):
    ...
    async def run(self, service):
        if service.protocol == 'tcp' and service.target.ipversion == 'IPv4':
            cmd = "whatweb --color=never --no-errors -a 3 -v {http_scheme}://{address}:{port} --log-json={scandir}/{protocol}{port}/{protocol}_{port}_{http_scheme}_whatweb.json"
            await service.execute(self, cmd)

    async def on_plugin_end(self, output, cmd, target=None, service=None):
        result_file = service.parse_string_vals("{scandir}/{protocol}{port}/{protocol}_{port}_{http_scheme}_whatweb.json")
            ...
            for result in results:
                for plugin_name, plugin in result.get('plugins', {}).items():
                    ...
                    for version in plugin.get('version', []):
                        data = "```bash\n$ %s\n```" % service.parse_string_vals("whatweb --color=never --no-errors -a 3 -v {http_scheme}://{address}:{port}")
