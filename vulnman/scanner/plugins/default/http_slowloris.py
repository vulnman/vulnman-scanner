...
class SlowlorisDOS(plugins.ServiceScanPlugin):
    _alias_ = "slowloris-dos"
    _tags = ["default", "http", "long"]
    _version_ = '0.0.1'
    toolname = "nmap"
    ...
    async def run(self, service):
        if service.protocol == "tcp":
            cmd = 'nmap --script=http-slowloris-check -p {port} {address} -oN "{scandir}/{protocol}' \
                  '{port}/{protocol}_{port}_{http_scheme}_nmap.txt" -oX "{scandir}/xml/' \
                  '{protocol}_{port}_{http_scheme}_nmap.xml"'
