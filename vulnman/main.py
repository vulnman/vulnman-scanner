...
from vulnman.config import configurable_keys, configurable_boolean_keys
from vulnman.core.assets.target import Target
...
async def run():
    # Find config file.
    if os.path.isfile(os.path.join(config["base_dir"], 'config.yaml')):
        config_file = os.path.join(config["base_dir"], 'config.yaml')
    ...
    if not args.config_file:
        unknown_help()
        logger.fail('Error: Could not find config.yaml')
